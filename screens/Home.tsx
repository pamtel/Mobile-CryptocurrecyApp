...
export const Home = ({ navigation }: Iprops): JSX.Element => {
  ...
  const renderHeader = () => {
    ...
    const renderItem = ({ item, index }: { item: ITrendingCurrencies, index: number }) => (
      <TouchableOpacity
        style={{
          width: 180,
      ...
      >
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          <View style={{ marginLeft: SIZES.base }}>
            <Text style={{ fontWeight: '700', ...FONTS.h2 }}>{item.currency}</Text>
            <Text style={{ color: COLORS.gray, ...FONTS.body3 }}>{item.code}</Text>
          </View>
        </View>
        {/* Value */}
        <View style={{ marginTop: SIZES.radius }}>
          <Text style={{ ...FONTS.h2, fontWeight: '600' }}>${item.amount}</Text>
          <Text style={{ color: item.type == 'I' ? COLORS.green : COLORS.red, ...FONTS.h3, fontWeight: '500' }}>
            {item.changes}
          </Text>
        </View>
      </TouchableOpacity>
      ...
      <View style={{ width: '100%', height: 290, ...styles.shadow }}>
        ...
        >
          ...
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: COLORS.white, ...FONTS.h3, fontWeight: '700' }}>Your Portfolio Balance</Text>
            <Text
              style={{
          ...
          >
            <Text style={{ marginLeft: SIZES.padding, color: COLORS.white, fontWeight: '700', ...FONTS.h2 }}>Trending</Text>
            <FlatList
  ...
  const renderNotice = () => {
    return (
      ...
      >
        <Text style={{ color: COLORS.white, fontWeight: '700', ...FONTS.h3 }}>Investing Safety</Text>
        <Text style={{ marginTop: SIZES.base, color: COLORS.white, ...FONTS.h4, lineHeight: 18 }}>
